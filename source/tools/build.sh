...
patch glibc-2.15/sysdeps/i386/configure < ../source/glibc-2.15-patch/glibc-2.15-cpuid-1.patch
patch glibc-2.15/sysdeps/i386/configure.in < ../source/glibc-2.15-patch/glibc-2.15-cpuid-2.patch
patch glibc-2.15/nptl/sysdeps/pthread/pt-initfini.c < ../source/glibc-2.15-patch/glibc-2.15-ifdef-1.patch
patch glibc-2.15/sysdeps/unix/sysv/linux/i386/sysdep.h < ../source/glibc-2.15-patch/glibc-2.15-ifdef-2.patch
...
#DO: kernel menuconfig 配置硬盘驱动为模块 : moduel
kernel_bzImage
kernel_modules
...
make CC=i686-none-linux-gnu-gcc
make install
croot
...
#EDIT: Makefile: ^IPV4_TARGETS=ping
#EDIT: Makefile: ^IPV6_TARGETS=tracepath6 traceroute6 ping6
#EDIT: Makefile: ^TARGETS=$(IPV4_TARGETS)
#EDIT: Makefile: ^USE_CAP=no
make CC=i686-none-linux-gnu-gcc
cp ping ${SYSROOT}/bin/
croot
...
#EDIT ${SYSROOT}/etc/ssh/sshd_config: ^UsePrivilegeSeparation no
#EDIT ${SYSROOT}/etc/ssh/sshd_config: ^PermitRootLogin yes
#EDIT ${SYSROOT}/etc/ssh/sshd_config: ^PermitEmptyPasswords yes
...
cd build/X7.7lib/
tar xvf ../../source/X7.7lib/pixman-0.28.0.tar.gz
cd pixman-0.28.0/
./configure --prefix=/usr
make install
croot
...
cd build/X7.7lib/
tar xvf ../../source/X7.7lib/xtrans-1.2.7.tar.bz2
cd xtrans-1.2.7/
./configure --prefix=/usr
make install
croot
...
cd build/X7.7lib/
tar xvf ../../source/X7.7lib/libXau-1.0.7.tar.bz2
cd libXau-1.0.7/
./configure --prefix=/usr
make install
croot
...
cd build/X7.7lib/
tar xvf ../../source/X7.7lib/libpthread-stubs-0.3.tar.bz2
...
cd build/X7.7lib/
tar xvf ../../source/X7.7lib/libxcb-1.8.1.tar.bz2
cd libxcb-1.8.1/
./configure --prefix=/usr
make
make install
...
# 安装 X 相关库和工具 : Mesa
./configure --prefix=/usr --with-dri-drivers=swrast,i915,i965 --disable-gallium-llvm --without-gallium-drivers --enable-32-bit
